...
const TransactionSummaryScreen = ({ route, navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={isProfit ? ['#134E5E', '#71B280'] : ['#4B1212', '#A33B3B']} style={styles.background} />
      
      <View style={styles.content}>
        <Ionicons name={isProfit ? "trending-up-outline" : "trending-down-outline"} size={80} color="#fff" />
        <Text style={styles.title}>{isProfit ? "Property Sold!" : "Sold at a Loss"}</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>+ ${finalSalePrice.toLocaleString()}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Original Purchase Price:</Text>
            <Text style={styles.value}>- ${purchasePrice.toLocaleString()}</Text>
          </View>
           <View style={styles.row}>
            <Text style={styles.label}>Total Investments:</Text>
            <Text style={styles.value}>- ${(totalInvestment - purchasePrice).toLocaleString()}</Text>
          </View>
          <View style={styles.separator} />
          <View style={styles.row}>
            <Text style={[styles.labelTotal, {color: isProfit ? '#43e97b' : '#e63946'}]}>
                {isProfit ? 'Net Profit:' : 'Net Loss:'}
            </Text>
            <Text style={[styles.valueTotal, {color: isProfit ? '#43e97b' : '#e63946'}]}>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: 'rgba(0,0,0,0.3)', padding: 25, borderRadius: 15, width: '100%', marginBottom: 40 },
  ...
  homeButton: { backgroundColor: 'rgba(255,255,255,0.9)', paddingVertical: 18, paddingHorizontal: 50, borderRadius: 30 },
